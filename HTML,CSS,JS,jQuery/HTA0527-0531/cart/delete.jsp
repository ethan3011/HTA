...
<%
	...
	// SAMPLE_CART_ITEMS 테이블에 대한 CRUD 기능이 구현된 CartItemDao객체를 획득한다.
	CartItemDao cartItemDao = CartItemDao.getInstance();
	// 아이템 번호에 해당하는 아이템정보를 조회한다.
	CartItem cartItem = cartItemDao.getCartItemByNo(itemNo);
	
	// 아이템정보가 조회되지 않으면 list로 되돌려 보낸다.
	if (cartItem == null){
		response.sendRedirect("list.jsp?fail=invalid");
		return;
	}
	// 아이템을 등록한 사람의 아이디와 로그인한 사용자자의 아이디가 동일하지 않으면
