...
<!-- 
	...
	2. 로그인된 사용자만 주문할 수 있다.
		...
		SessionSavedUser가 null 이면 "../user/loginform.jsp?fail=deny&job=주문하기"를 재요청하게 한다.
		
	3. 자신의 장바구니에 저장된 아이템만 주문할 수 있다.
		CartItem item = cartItemDao.getCartItemByNo(itemNo);
		item의 userId와 로그인한 사용자의 userId가 다른 경우 장바구니 list.jsp?fail=deny를 재요청하게 한다.
		 	
	4. 주문싯점에 재고가 없는 아이템은 주문할 수 없다.
		Product product = ProductDao.getProductByNo(item.getProductNo());
		product의 재고량이 item의 amount 보다 작으면 해당 상품은 주문되지 않는다.
	
	5. 주문이 완료되면 상품의 재고를 감소시킨다.
		product.setStock(product.getStock() - cartItem.getAmount());
		ProductDao객체의 updateProduct(product)를 실행해서 변경된 재고량을 데이터베이스에 반영한다.
	...
	7. 주문이 끝나면 현재 주문정보를 확인하는 페이지를 요청하는 URL을 응답으로 보낸다.
