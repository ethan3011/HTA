...
<%
	...
	/*
		클라이언트 전용의 세선객체가 JSP 내장객체를 저장하는 session 참조변수에 연결되는 과정
		1. 요청객체의 요청헤더정보에서 클라이언트가 전달한 세션아이디를 조회한다.
		2. 세션아이디에 해당하는 세션객체를 서버에서 검색한다.
		3. 세션아이디에 해당하는 세션객체가 검색되면 해당 세션객체를 session 참조변수에 대입한다.
		4. 개발자가 작성한 JSP 코드에서 session참조변수를 이용해서 세션객체에 접근할 수 있다.
	*/
	
	/*
		login.jsp에서는 사용자 인증이 완료되면 세션객체에 사숑자정보를 저장한다.
		session.setAttribute("LOGINED_USER_INFO", user);	// "LOGINED_USER_INFO" 이름으로 User객체 저장
		
		* 사용자 인증이 완료된 이후에 실행되는 모든 JSP에서는
		  login.jsp에서 세션객체에서 저장해둔 사용자정보를 꺼낼 수 있다.
		  User sessionSavedUser = (User) session.getAttribute("LOGINED_USER_INFO");
		* 단, 사용자 인증이 완료되는 않은 경우에는
		  세션객체에서 저장된 사용자 정보가 없기 때문에
		  User sessionSavedUser = (User) session.getAttribute("LOGGINED_USER_INFO");를 실행하면 
		  sessionSavedUser에 대입되는 값은 null 이다.
	*/
	...
	// 클라이언트 전용의 세션객체에서 사용자정보가 조회되지 않으면 로그인하지 않은 사용자 이다.
	...
	// CartItem객체를 생성해서 사용자 아이디, 상품번호를 저장한다.
