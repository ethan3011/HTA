...
public class CartItemDao {
	...
	/**
	 * 아이템번호를 전달받아서 번호에 해당하는 SAMPLE_CART_ITEM에서 아이템정보를 삭제한다.
	 * @param itenNo
	 */
	public void deleteCartItem(int itenNo) {
		SqlSession session = sqlSessionFactory.openSession(true);
		session.delete("deleteCartItem", itenNo);
		session.close();
	}
	 ...
	 * @return 장바구니 아이템 목록
	 */
	public List<CartItemListDto> getCartItemByUserId(String userId){
		SqlSession session = sqlSessionFactory.openSession();
		List<CartItemListDto> items = session.selectList("getCartItemByUserId", userId);
		session.close();
		return items;
	}
}
